using HexChat.Business.Attributes;
...
namespace HexChat.Business.HandlerContainers {
    public class MessageHandlerContainer {
        ...
        internal MessageHandlerContainer(ClientBusiness client)
            : this(client, typeof(MessageHandlerContainer).Assembly) { }

        internal MessageHandlerContainer(ClientBusiness client, Assembly assembly) {
            this.client = client;
        ...
        public void RegisterCustomMessageHandler(Type type) {
            ...
            if (!IsMessageConstructorValid(messageType)) {
                throw new InvalidOperationException($"{messageType.Name} must have a constructor with exactly one parameter of type {nameof(ParsedIRCMessageModel)}.");
            }
        ...
        private bool IsMessageConstructorValid(Type messageType) {
            return messageType.GetConstructor(
                    ...
                    binder: null,
                    types: new[] { typeof(ParsedIRCMessageModel) },
                    modifiers: null) != null;
        }
        ...
        internal async Task<object> HandleAsync(ParsedIRCMessageModel parsedIRCMessage) {
        ...
        internal async Task<T> InvokeHandler<T>(ParsedIRCMessageModel parsedIRCMessage, MessageHandler messageHandler)
            where T : class {
